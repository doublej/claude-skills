...
`automation.turn_off` disables triggers. Entity remains in state machine with state `off`.
...
Disabling via UI: Settings → Automations → open automation → ⋮ → Settings → Enabled toggle.

Sets `disabled_by: user` in `core.entity_registry`. Entity is removed from state machine entirely. Requires an `id:` field in the automation YAML.

### AVOID: `enabled: false` in automations.yaml

`enabled:` is NOT a valid top-level key. HA rejects it during schema validation — the automation loads as `unavailable`.
